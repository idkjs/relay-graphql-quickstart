...
module Enum_EnterpriseMembershipType: {
  type t = [
    | `ALL
    | `ADMIN
    | `BILLING_MANAGER
...
module Enum_EnterpriseOrderField: {
  type t = [ | `NAME | `FUTURE_ADDED_VALUE__];
  type wrapped;
  let unwrap: wrapped => t;
  let wrap: t => wrapped;
...
module Enum_ReportedContentClassifiers: {
  type t = [
    | `SPAM
    | `ABUSE
    | `OFF_TOPIC
...
module Enum_OauthApplicationRevokeTokensAuditEntryState: {
  type t = [
    | `ACTIVE
    | `SUSPENDED
    | `PENDING_DELETION
...
module Enum_PullRequestPubSubTopic: {
  type t = [
    | `UPDATED
    | `MARKASREAD
    | `HEAD_REF
...
module Enum_RegistryPackageFileState: {
  type t = [ | `NEW | `UPLOADED | `FUTURE_ADDED_VALUE__];
  type wrapped;
  let unwrap: wrapped => t;
  let wrap: t => wrapped;
} = {
...
module Enum_RepositoryCollaboratorAffiliation: {
  type t = [ | `ALL | `OUTSIDE | `FUTURE_ADDED_VALUE__];
  type wrapped;
  let unwrap: wrapped => t;
  let wrap: t => wrapped;
} = {
